...
import ExclamationCircle from "@/components/icons/heroicons/outline/exclamation-circle";
...
import { passwordResetAction } from "../auth-actions";
import { AuthForm } from "../auth-forms";

export const dynamic = "force-dynamic";

const resetMetrics = [
  { label: "Reset token validity", value: "15 min" },
  { label: "Session refresh", value: "Instant" },
  { label: "Protected devices", value: "All" },
];

const resetGuardrails = [
  "Choose a strong password that has not been used on another workspace",
  "Resetting your password can invalidate existing high-risk sessions after confirmation",
  "Every recovery event is captured in the dashboard audit trail for operator review",
];
...
export default async function PasswordResetPage() {
  ...
  const verification = user
    ? {
        valid: true as const,
        message: "Your reset link is still valid. Choose a new password below.",
      }
    : {
          ...
          <div className="hidden lg:block">
            <div className="max-w-xl space-y-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-indigo-200 bg-white/80 px-4 py-2 text-sm font-semibold text-indigo-700 shadow-sm backdrop-blur">
                ...
                Verified reset with audit-ready protection
              </div>

              <div className="space-y-4">
                <h1 className="text-5xl font-extrabold leading-tight tracking-tight text-slate-950">
                  Set a new password and re-enter your workspace securely.
                </h1>
                <p className="max-w-lg text-lg leading-8 text-slate-600">
                  This reset screen keeps the same premium auth language: clean
                  hierarchy, confident guidance, and trust cues for every
                  sensitive step.
                </p>
              </div>

              <div className="grid gap-4 sm:grid-cols-3">
                {resetMetrics.map((item) => (
                  <div
                    key={item.label}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="text-xl font-bold tracking-tight text-slate-900">
                      Guardrails for a safer return to work
                    </h2>
                  </div>
                </div>

                <ul className="mt-6 space-y-4">
                  {resetGuardrails.map((item) => (
                    <li
                      key={item}
            ...
            <div className="relative rounded-[2rem] border border-white/80 bg-white/90 p-6 shadow-2xl shadow-slate-200/70 backdrop-blur sm:p-8">
              <div className="mb-8 text-center lg:text-left">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Open the link from your email to reach this page with an
                  active reset session, then set a new password.
                </p>
              </div>
                ...
                {verification.valid ? (
                  ...
                  >
                    ...
                    <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm font-medium leading-6 text-slate-600">
                      ...
                      />
                      <span>
                        Sign out other active sessions after this password reset
                        completes.
                      </span>
                    </label>
                  </AuthForm>
